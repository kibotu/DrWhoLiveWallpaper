...
public class Tunnel3D {
    ...
    public Tunnel3D(int revolution, int depth) {
        ...
        colors_buffer = makeFloatBuffer(new float[]{
                0, 0, 0,
                0, 1, 0,
                1, 1, 0});
    ...
    private void buildBuffers() {
        ...
        colors_direct = ByteBuffer.allocateDirect(colors.length * 4);
    ...
    public void render(float depth) {
        ...
        glVertexPointer(3, GL_FLOAT, 0, vertices_buffer);
        glTexCoordPointer(2, GL_FLOAT, 0, texture_buffer);
        glColorPointer(4, GL_FLOAT, 0, colors_buffer);

        int dy = 0;
        int nf = (nx + 1) << 1;
        faces_buffer.position(0);
        for (int y = 0; y < (ny - 1); y++) {
            glDrawElements(GL_TRIANGLE_STRIP, nf, GL_UNSIGNED_SHORT, faces_buffer);
            dy += nf;
